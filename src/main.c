...
int	main(int ac, char **av)
{
	...
	data.monster.sprite.texture = malloc(sizeof(t_img));
	if (!data.monster.sprite.texture)
		ft_game_exit(&data, "Erreur : Malloc de la texture du bot a échoué.");
	ft_init_bot_tex(&data, data.monster.sprite.texture);
	ft_init_bfs(&data);
